      ...
      def timesync
        ...
        @hosts.each do |host|
          ...
          if host['platform'].include? 'solaris-10'
            host.exec(Command.new("sleep 10 && ntpdate -w #{@options[:ntpserver]}"))
            ...
            host.exec(Command.new("w32tm /config /manualpeerlist:#{@options[:ntpserver]} /syncfromflags:manual /update"))
            ...
            until success do
              ...
              if host.exec(Command.new("ntpdate -t 20 #{@options[:ntpserver]}")).exit_code == 0 
                success=TRUE 
              end
            end
