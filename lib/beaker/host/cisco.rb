...
module Cisco
  class Host < Unix::Host
    ...
    def validate_setup
      msg = nil
      if self[:platform] =~ /cisco_nexus/ && self[:vrf] == nil
        msg = 'Cisco Nexus hosts must be provided with a :vrf value.'
      end
      if !msg && self[:platform] =~ /cisco/ && self[:user] == nil
        msg = 'Cisco hosts must be provided with a :user value, with sudo privileges.'
      end
