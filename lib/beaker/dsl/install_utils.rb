...
module Beaker
  module DSL
    ...
    module InstallUtils
      ...
      # This method is a fix for PUP-2365 when a windows agent cannot download
      # modules from the forge because it doesn't have the appropriate
      # certificates installed to validate the https connection.
      #
      # This method installs the certifcates required in order to that functionality
      # to work as expected.
      #
      # @param [Host] host             A host object
      #
      def install_forge_cert(host)
        geotrust_global_ca = <<-CERT.gsub(/^ {8}/, '')
        -----BEGIN CERTIFICATE-----
        ...
        hosts.each do |host|
          if host['platform'] =~ /windows/
            install_cert_on(host, 'geotrust_global_ca', geotrust_global_ca)          
          end
        end
      end
      ...
      #                     
      def install_cert_on(host, cert_name, cert)
