...
module Beaker
  module DSL
    ...
    module Helpers
      ...
      #Fetch file_name from base_url into dst_dir.
      #
      # @!visibility private
      ...
      #Installs development repository configuration on specified host using
      #"push" method.
      # @param [Host] host An object implementing {Beaker::Hosts}'s
      #                    interface.
      # @param [String] package_name A string naming a valid subdirectory of
      #                              http://builds.puppetlabs.lan/ and which
      #                              corresponds to a PuppetLabs
      #                              internally-built project.
      # @param [String] build_version A string corresponding to a specific build
      #                               of package_name. For JVM projects such as
      #                               puppetdb this will often correspond to a
      #                               timestamp used to tag an ephemeral git
      #                               repository. For Ruby and C++ projects this
      #                               very likely will correspond to a SHA
      #                               representing a commit in the project's
      #                               main git repository.
      # @param [String] repo_configs_dir A string representing a local path at
      #                                  which the repo config will be stored
      #                                  prior to being pushed to the SUT.  The
      #                                  default value is 'tmp/repo_configs'; if
      #                                  './tmp' does not exist, it will be
      #                                  created. No guarantees are made about
      #                                  files stored in this directory; they
      #                                  may be overwritten on the next run of
      #                                  this method.
      #
      # @example
      #  install_dev_repo(master, 'puppetdb', '1.6.3')
      #
      # @api dsl
      def install_dev_repo (host, package_name, build_version, repo_configs_dir='tmp/repo_configs')
        ...
        when /^(fedora|el-|centos)/
          ...
          repo = fetch(
            "http://builds.puppetlabs.lan/%s/%s/repo_configs/rpm/" % [package_name, build_version],
            repo_filename,
        ...
        when /(debian|ubuntu)/
          list = fetch(
            "http://builds.puppetlabs.lan/%s/%s/repo_configs/deb/" % [package_name, build_version],
            "pl-%s-%s-%s.list" % [package_name, build_version, codename],
            platform_configs_dir
          )
          ...
          logger.notify("No repository installation step for #{platform} yet...")
        end
      end
