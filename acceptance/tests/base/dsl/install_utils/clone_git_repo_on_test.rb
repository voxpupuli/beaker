...
test_name 'Clone from git' do
  ...
  hosts.each do |host|
    ...
    step 'should find fork name from the correct environment variable' do
      results = clone_git_repo_on(host, "#{testdir}", extract_repo_info_from(build_git_url('puppet', nil, nil, 'git')))
    ...
    step 'should clone hiera from correct fork' do
      results = clone_git_repo_on(host, "#{testdir}", extract_repo_info_from(build_git_url('hiera', nil, nil, 'git')))
