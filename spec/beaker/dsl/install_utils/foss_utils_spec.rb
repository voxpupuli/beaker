...
describe ClassMixedWithDSLInstallUtils do
  ...
  describe '#install_puppet_agent_dev_repo_on' do
    ...
    it 'runs the correct install for osx platforms (newest link format)' do
      ...
      expect(subject).to receive(:link_exists?).with(/\/puppet-agent\/0.1.0\/repos\/apple\/10.9\/PC1\/x86_64\//).and_return(true).twice
      expect(subject).to receive(:fetch_http_file).once.with(/\/apple\/10.9\/PC1\/x86_64$/, 'puppet-agent-0.1.0-1.osx10.9.dmg', /\/osx$/)
    ...
    it 'runs the correct install for osx platforms (new link format)' do
      ...
      expect(subject).to receive(:link_exists?).with(/\/puppet-agent\/0.1.0\/repos\/apple\/10.9\/PC1\/x86_64\//).and_return(false, true)
    ...
    it 'runs the correct install for osx platforms (old link format)' do
      ...
      expect(subject).to receive(:link_exists?).with(/\/puppet-agent\/0.1.0\/repos\/apple\/10.9\/PC1\/x86_64\//).and_return(false, false)
