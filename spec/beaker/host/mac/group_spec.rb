...
EOS
  ...
  describe '#group_present' do
    ...
    it 'runs correct create command if group does not exist' do
      ...
      subject.should_receive( :execute ).with("dscl . create /Groups/#{name} && dscl . create /Groups/#{name} PrimaryGroupID #{gid}").once.ordered
  ...
  describe '#group_absent' do
    ...
    it 'calls execute to run logic' do
      name = "main_one"
      subject.should_receive( :execute ).once.with("if dscl . -list /Groups/#{name}; then dscl . -delete /Groups/#{name}; fi", {})
