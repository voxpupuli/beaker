...
module Beaker
  describe SshConnection do
    ...
    describe '#close' do
      ...
      it 'calls ssh shutdown if ssh close fails with an unexpected IOError' do
        mock_ssh = Object.new
        allow( mock_ssh ).to receive( :close ) { raise IOError.new('pants') }
        expect( Net::SSH ).to receive( :start ).with( host, user, ssh_opts) { mock_ssh }
        connection.connect

        expect( mock_ssh ).to receive( :shutdown! ).once
        expect{ connection.close }.to raise_error(IOError)
        expect( connection.instance_variable_get(:@ssh) ).to be_nil
      end

      it 'calls ssh shutdown if ssh close fails with an IOError \'closed stream\'' do
        mock_ssh = Object.new
        allow( mock_ssh ).to receive( :close ) { raise IOError.new('closed stream') }
        expect( Net::SSH ).to receive( :start ).with( host, user, ssh_opts) { mock_ssh }
        connection.connect

        expect( mock_ssh ).to receive( :shutdown! ).never
